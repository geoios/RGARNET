...
%% Functions£ºAcoustic ray tracking core function, calculating the propagation time, depth, and horizontal displacement of each layer
%% inputs:
  % + PF          %   sound profile
...
for i = 1 : m-1
    ...
    if i == 1
        ...
        RayInf.c0 = c0;
        P00 = RayInf.alfa0/RayInf.c0;    % **********Check**********
    end
    ...
    diff(i) = cos_alfa0/cv -  RayInf.alfa0/RayInf.c0;  % ********Check********
    
    
  %×îºóÒ»²ã
  if     H < inf       % Regarding the Depth as the cut-off condition
    TotalZ = sum(zz); if TotalZ >= H; V(3) = H - (TotalZ - zz(i)); end                     % Calculate the propagation depth of the last layer
  ...
  elseif   Y < inf       % Regarding the Horizontal displacement as the cut-off condition
