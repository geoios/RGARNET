...
StnName  = 'MYGI';
StnPath  = '.';
% The directory of observation files.
...
for StaIndex = 1:length(Filename)
    ...
    for DataNum = 1:FileStruct.FileNum
        ...
        % Obtain horizontal coordinate data
        ...
        % Acquire the coordinates of the full sequence.
        EachPosList{DataNum} = EachPos;

        % Calculate the horizontal area of the corresponding array.
        S(DataNum) = polyarea(EachPos(:,1),EachPos(:,2));

        % Calculate the corresponding elevation change.
...
xlim([2012,2021]);
% Set the size of the drawing sheet.
...
Mu3 = [0.001,0.005,0.01,0.05,0.1,1,3.0088,5,10,50,100,500,1000]; %
[Value_Min2,Value_Min3] = deal(zeros(1,length(Mu3)));
for Loop = 1:length(Mu3)
    % Re - calculate and determine the hyper - parameters according to the minimum - index sequence.
    [Value_Min2(Loop),Value_Min3(Loop)] = Cal_Earth_adhere_dK(log10(Mu3(Loop)),TabList_eff,StnName,StationTime,K_Ref_mean,Base_S_H); 
end

% Draw the results of factor changes.
figure(234)
...
load(savefile,'TabList_eff','RESData_tmp','RESData','StationTime','K_Ref_mean');
...
% Draw a double - axis graph. The left side is blue representing area changes, and the right side is red representing elevation changes.
figure(365)
colororder({'b','r'})
yyaxis left
...
figure(365)
yyaxis left
text(2015,-0.02,['\it{dS} = ',num2str(dS_PolyMP(1),'%4.4f'),'t + ',num2str(dS_PolyMP(2),'%4.2f')],'Color','blue','FontSize',10,'FontName','Times new roman');
yyaxis right
text(2018,0.05,['\it{dU} = ',num2str(dH_PolyMP(1),'%4.4f'),'t + ',num2str(dH_PolyMP(2),'%4.2f')],'Color','red','FontSize',10,'FontName','Times new roman');
